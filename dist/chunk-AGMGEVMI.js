      ...
      for (const mod of modules) {
        ...
        if (aliases[mod.name]) {
          ...
          if (conflictSet.has(lowerName)) {
            ...
            warnings.push(
              `Module "${mod.name}" conflicts with a package in node_modules. Alias auto-renamed to "${alias}". Use the "aliases" option to set a custom name.`
            );
          } else {
...
function syncTsConfigFile(root, modulesDir, modules, tsConfigPath) {
...
function parseCurrentIncludes(text) {
  try {
